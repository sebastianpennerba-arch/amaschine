// ======================================================
// AdStream Analytics – Clean & Professional
// ======================================================
...
function setupMetaButton() {
  ...
  btn.addEventListener("click", () => {
    ...
    window.open(authUrl, "metaAuth", `width=${w},height=${h},left=${left},top=${top}`);
  });
...
function renderOverviewEmpty() {
  const grid = document.getElementById("overviewGrid");
  if (!grid) return;
  grid.innerHTML = `<div style="grid-column:1/-1; color:#6B7280;">Keine Insights verfügbar.</div>`;
}
...
function renderFunnelEmpty() {
  const el = document.getElementById("funnelSteps");
  if (!el) return;
  el.innerHTML = `<div style="grid-column:1/-1; color:#6B7280;">Keine Funnel-Daten verfügbar.</div>`;
}
...
function renderKPIsEmpty() {
  const el = document.getElementById("kpiGrid");
  if (!el) return;
  el.innerHTML = `<div style="grid-column:1/-1; color:#6B7280;">Keine KPI-Daten verfügbar.</div>`;
}
...
function renderCreatives() {
  ...
  if (!items.length) {
    grid.innerHTML = `<div style="grid-column:1/-1; text-align:center; color:#6B7280;">Keine Creatives gefunden.</div>`;
    return;
  }
