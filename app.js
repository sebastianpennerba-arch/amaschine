...
function getEffectiveBrandOwnerName() {
  if (AppState.meta && AppState.meta.user?.name) {
...
function populateBrandSelect() {
  ...
  // auto default – doppelte Sicherheit
...
function renderNav() {
  ...
  Object.keys(modules).forEach((key) => {
    if (key === "settings") return; // kein Settings-Eintrag oben
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (metaBtn) metaBtn.addEventListener("click", toggleMetaConnection);
  updateMetaStatusUI();
  updateSystemHealthUI();
  updateCampaignHealthUI();
  ...
  if (settingsBtn) {
    settingsBtn.addEventListener("click", () => {
      navigateTo("settings");
      showToast("Settings geöffnet.", "info");
    });
  }
